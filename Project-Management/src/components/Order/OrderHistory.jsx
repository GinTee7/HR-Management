...
const OrderHistory = () => {
  ...
  const filteredItems =
    filter === "all"
      ? cartItems
      : cartItems.filter((item) => item.status === filter);
    ...
    <div className="container py-10 w-full bg-[#F2F8FC] max-w-none pt-32">
      ...
      <div className="flex gap-4 mb-6">
        ...
        <button
          ...
          className={`px-4 py-2 rounded ${
            filter === "chờ xác nhận" ? "bg-blue-500 text-white" : "bg-gray-200"
          }`}
        >
          Chờ xác nhận
        </button>
        ...
        <button
          ...
          className={`px-4 py-2 rounded ${
            filter === "đã giao" ? "bg-blue-500 text-white" : "bg-gray-200"
          }`}
        >
          Đã giao
        </button>
        <button
          ...
          className={`px-4 py-2 rounded ${
            filter === "đã hủy" ? "bg-blue-500 text-white" : "bg-gray-200"
          }`}
        >
          Đã hủy
        </button>
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {filteredItems.map((item) => (
          ...
          >
            {/* Product Image */}
            <div className="h-48 overflow-hidden bg-white rounded">
              <img
                src={item.img}
                alt={item.name}
