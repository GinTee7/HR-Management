...
const LoginPage = () => {
    ...
    // ✅ Kiểm tra nếu đã có token thì chuyển hướng
    useEffect(() => {
    ...
    const handleSubmit = async e => {
        ...
        try {
            ...
            if (token) {
                localStorage.setItem('token', `Bearer ${token}`); // ✅ Lưu token dạng Bearer
                localStorage.setItem('roleName', roleName); // ✅ Lưu roleName để quản lý quyền

                // ✅ Chuyển hướng theo roleName
                switch (roleName) {
                    case 'ADMIN':
                        navigate('/admin');
                        break;
                ...
                setError(t('Token not received, please try again.'));
            }
        } catch (err) {
                ...
                <div className='flex flex-col justify-center w-full p-8 bg-white md:w-1/2'>
                    ...
                    >
                        <input
                            ...
                            type='text'
                            name='userName' // ✅ Gửi đúng field "userName"
