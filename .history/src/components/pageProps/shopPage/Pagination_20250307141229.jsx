    ...
    return (
        <div className='grid grid-cols-1 gap-6 p-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 bg-[#F3F7FC]'>
            {currentItems.map(item => (
                <div
                    key={item._id}
                    className='relative flex flex-col items-center overflow-hidden transition-transform transform bg-white border border-gray-300 shadow-md rounded-xl hover:scale-105 hover:shadow-xl w-100 h-[450px]'
                    style={{ borderColor: dominantColors[item._id] || '#ddd' }}
                >
                    {/* Ảnh sản phẩm */}
                    <div className='flex items-center justify-center w-full p-4 bg-white h-72'>
                        <img
