...
const Items = ({ currentItems }) => {
    ...
    useEffect(() => {
        currentItems.forEach(item => {
            getDominantColor(item.img || "https://via.placeholder.com/300", item._id);
        });
    }, [currentItems]);
    ...
    return (
        <div className="grid grid-cols-1 gap-8 p-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {currentItems.map(item => (
                <div
                    key={item._id}
                    className="relative flex flex-col items-center overflow-hidden transition-transform transform bg-white shadow-md rounded-3xl hover:shadow-lg hover:scale-105"
                    style={{ border: `2px solid ${dominantColors[item._id] || "#ddd"}` }}
                >
                    {/* Ảnh sản phẩm */}
                    <div className="flex items-center justify-center w-full p-4 bg-gray-100 h-80 rounded-t-3xl">
                        <img
                            className="object-contain w-full h-full max-w-[85%] max-h-[85%] rounded-md"
                    ...
                    <div className="flex flex-col items-center p-5 text-center">
                        ...
                        <Link
                            to="/product"
                            className="flex items-center gap-2 px-4 py-2 mt-4 text-sm text-white transition-all bg-blue-600 rounded-lg hover:bg-blue-500"
                        >
    ...
    return (
        <div className='grid grid-cols-1 gap-6 p-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'>
            {currentItems.map(item => (
                ...
                >
                    {/* Ảnh sản phẩm (Hiển thị toàn bộ, không bị crop) */}
                    <div className='flex items-center justify-center w-full h-64 bg-gray-100 rounded-t-3xl'>
                        <img
                            className='object-contain w-full h-full max-w-[90%] max-h-[90%] rounded-t-3xl'
                            src={item.img || 'https://via.placeholder.com/300'}
                            alt={item.productName || 'Hình ảnh sản phẩm'}
                        />
                    </div>
