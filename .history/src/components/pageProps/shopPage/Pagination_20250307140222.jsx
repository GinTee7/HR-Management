    ...
    return (
        <div className='grid grid-cols-1 gap-6 p-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'>
            {currentItems.map(item => (
                <div
                    key={item._id}
                    className='relative flex flex-col items-center overflow-hidden transition-transform transform bg-white shadow-md rounded-3xl hover:shadow-xl hover:scale-105'
                    style={{
                        border: `2px solid ${
                            dominantColors[item._id] || '#ddd'
                        }`,
                        backgroundColor: '#F9F9F9' // Nền sáng giống ảnh mẫu
                    }}
                >
                    {/* Ảnh sản phẩm */}
                    <div className='flex items-center justify-center w-full p-6 bg-white h-80 rounded-t-3xl'>
                        <img
                            className='object-contain w-full h-full max-w-[80%] max-h-[80%] rounded-md'
