...
const Items = ({ currentItems }) => {
    ...
    return (
        <div className='grid grid-cols-1 gap-6 p-6 bg-white sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'>
            {currentItems.map(item => (
                <div
                    key={item._id}
                    className='relative flex flex-col items-center overflow-hidden transition-all bg-white border border-gray-300 shadow-lg rounded-2xl hover:scale-105 hover:shadow-2xl w-full max-w-[450px] h-[500px]'
                    style={{ borderColor: dominantColors[item._id] || '#ddd' }}
                >
                    {/* Ảnh sản phẩm */}
                    <div className='relative flex items-center justify-center w-full p-4 bg-gray-100 h-100'>
                        <img
